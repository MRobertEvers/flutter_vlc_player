...
void VlcPlayerApiSetup(id<FlutterBinaryMessenger> binaryMessenger, id<VlcPlayerApi> api) {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [FlutterBasicMessageChannel
        messageChannelWithName:@"dev.flutter.pigeon.VlcPlayerApi.startRecord"
        binaryMessenger:binaryMessenger];
    if (api) {
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        TextureMessage *input = [TextureMessage fromMap:message];
        FlutterError *error;
        BooleanMessage *output = [api startRecord:input error:&error];
        callback(wrapResult([output toMap], error));
      }];
    }
    else {
  ...
  }
  {
    FlutterBasicMessageChannel *channel =
      [FlutterBasicMessageChannel
        messageChannelWithName:@"dev.flutter.pigeon.VlcPlayerApi.stopRecord"
        binaryMessenger:binaryMessenger];
    if (api) {
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        TextureMessage *input = [TextureMessage fromMap:message];
        FlutterError *error;
        SnapshotMessage *output = [api stopRecord:input error:&error];
        callback(wrapResult([output toMap], error));
      }];
    }
    else {
